...
Use `zcat` for 2.4 in order to display correct number of lines. -3pts
...
-   Are branches (`master` and `develop`) correctly set up? Is the hw submission put into the `master` branch? Homework was not put into `master` branch on time. Try to do this in the future. 
...
For bash commands, only enforce the 80-character rule. **Not followed. Violated 5+ times. -10pt**
